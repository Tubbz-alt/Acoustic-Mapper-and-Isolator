...
Problem_Management_Workflow2UI <- function(id){
...
Problem_Management_Workflow2 <- function(input, output, session, vals = NULL) {
  ...
  output$equipment = renderUI({
    selectInput(ns('equipment'), 'Equipment ID', c("Fan 1","Fan 2"), selected = "Fan 2")
  })
  ...
  output$plot1 <- renderPlot({
    ...
    Healthy_Data_Sample <- as.data.frame(vals$please[((i-1)*10000):(i*10000),])
  ...
  output$plot2 <- renderPlot({
    ...
    xfft <- as.data.frame(vals$please[((i-1)*10000):(i*10000),])
